...
const string CoreWorkload::MEMPOOL_PROPERTY = "mempool";
const string CoreWorkload::MEMPOOL_DEFAULT = "1";

void CoreWorkload::Init(const Properties &p) {
	...
	double readmodifywrite_proportion = std::stod(
			p.GetProperty(READMODIFYWRITE_PROPORTION_PROPERTY, READMODIFYWRITE_PROPORTION_DEFAULT));
	...
	read_all_fields_ = Util::StrToBool(
			p.GetProperty(READ_ALL_FIELDS_PROPERTY, READ_ALL_FIELDS_DEFAULT));
	write_all_fields_ = Util::StrToBool(
	...
	if (scan_len_dist == "uniform") {
		...
		throw Util::Exception("Distribution not allowed for scan length: " + scan_len_dist);
	}
	...
	useMempoolCache = (0 != std::stoi(p.GetProperty(MEMPOOL_PROPERTY, MEMPOOL_DEFAULT)));
}

Core::Generator<uint64_t> *CoreWorkload::GetFieldLenGenerator(const Properties &p) {
	...
	if (field_len_dist == "constant") {
		...
		throw Util::Exception("Unknown field length distribution: " + field_len_dist);
	}
}

void CoreWorkload::BuildValues(std::vector<DB::DB_BASE::KVPair> &values) {
	for (int i = 0; i < field_count_; ++i) {
		DB::DB_BASE::KVPair pair;
		pair.first.append("field").append(std::to_string(i));
		pair.second.append(field_len_generator_->Next(), Util::RandomPrintChar());
		values.push_back(pair);
	}
}

void CoreWorkload::BuildUpdate(std::vector<DB::DB_BASE::KVPair> &update) {
	DB::DB_BASE::KVPair pair;

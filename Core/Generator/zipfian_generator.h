...
public:
	...
	ZipfianGenerator(uint64_t num_items) :
			num_items_(num_items), base_(0), theta_(kZipfianConst), zeta_n_(0), n_for_zeta_(
					0) {
		assert(num_items_ >= 2 && num_items_ < kMaxNumItems);
		zeta_2_ = Zeta(2, theta_);
		alpha_ = 1.0 / (1.0 - theta_);
